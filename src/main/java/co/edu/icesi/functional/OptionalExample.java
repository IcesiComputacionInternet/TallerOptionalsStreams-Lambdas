...
public class OptionalExample {
    ...
    public String obtainUserStreet(IcesiUser icesiUser) {
        Optional<IcesiUser> showStreet = Optional.ofNullable(icesiUser);
        return showStreet.map(IcesiUser::getAddress).map(IcesiAddress::getStreet).orElseThrow(() -> new RuntimeException("Couldn't get the street"));
    }
    ...
    public String obtainUserDescription(IcesiUser icesiUser) {
        Optional<IcesiUser> showDescription = Optional.ofNullable(icesiUser);
        return showDescription.map(IcesiUser::getAddress).map(IcesiAddress::getDescription).orElseGet(() -> "default description");
    }
