...
public class OptionalExample {
    ...
    public String obtainUserStreet(IcesiUser icesiUser) {
        ...
        return optionalIcesiUser.map(IcesiUser::getAddress).map(IcesiAddress::getStreet).orElseThrow(() -> new RuntimeException("Couldn't get the street"));
    }
    ...
    public String obtainUserDescription(IcesiUser icesiUser) {
        Optional<IcesiUser> optionalIcesiUser = Optional.ofNullable(icesiUser);
        String result = optionalIcesiUser.map(IcesiUser::getAddress).map(IcesiAddress::getDescription).orElseGet(() -> "default description");
        return result;
    }
